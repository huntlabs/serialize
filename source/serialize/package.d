...
public import serialize.write;
public import serialize.read;
...
version(unittest)
{
	...
	void testTAT()
	{
		...
		WriteStream strem = WriteStream(512);
		TAT.serialize(tat,&strem);
		ubyte[] data = strem.data();
	...
	ubyte[] seriTA(ref TA t)
	{
		WriteStream strem = WriteStream(256);
		strem.startStruct();
		strem.write!Date(t.de);
		strem.write!(ubyte[])(t.data);
		strem.write!(string)(t.str);
		strem.write!(double)(t.db);
		strem.write!(long)(t.lo);
		strem.write!(bool)(t.bl);
		strem.write!(uint)(t.ui);
		//	strem.append(seriTT(t.ta,&strem));
		seriTT(t.ta,&strem);
		strem.startArray!int();
		foreach(ref i; t.iarry)
		{
			strem.write!int(i);
		}
		strem.endArray();
		strem.endStruct();
		return strem.data(true);
	}
